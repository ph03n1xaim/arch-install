...
function main(){
	...
	baseutilspkg="sudo man bash-completion reflector tmux tlp ffmpeg efibootmgr neofetch"
	microcodepkg="amd-ucode"
	filesystempkg="btrfs-progs"
	networkpkg="wpa_supplicant"
	audiopkg="alsa-utils pulseaudio pulseaudio-alsa"
	bluetoothpkg="bluez bluez-utils"
	xorgpkg="xorg-server xorg-xinit xorg-server-xwayland"
...
function setup(){
	case "$setupoption" in
		"-h" | "--help")
			...
			echo ' -en, --network        enable network manager  (14)'
			echo ' -et, --tlp            enable tlp              (15)'
			echo ' -d, --displaymanager  enable display manager  (16)'
			echo ' -g, --graphics        configure graphics      (17)'
			echo ' -a, --aur             install aur helpers     (18)'
			echo ' -b, --boot            install bootloader      (19)'
			echo ' -sb, --secureboot     enable secureboot       (20)'
			echo ' -q, --quit            quit installer          (21)'
			echo ''
		...
		"-eb" | "--bluetooth") archenablebluetooth
			;;
		"-en" | "--network") archenablenetwork
			;;
		"-et" | "--tlp") archenabletlp
			;;
		"-d" | "--displaymanager") archenabledisplaymanager
			;;
...
function continueinstall(){
	case "$continuestate" in
		...
		13) archenablebluetooth
			continuestate=13
			;&
		14) archenablenetwork
			continuestate=14
			;&
		15) archenabletlp
			continuestate=15
			;&
		16) archenabledisplaymanager
			continuestate=16
			;&
		17) archgraphics
			continuestate=17
			;&
		18) archinstallaurhelpers
			continuestate=18
			;&
		19) archinstallbootloader
			continuestate=19
			;&
		20) archsecureboot
			continuestate=20
...
function updatescript(){
	curl -L "https://www.tinyurl.com/aim-arch-install" -o $1 && exit 0
}
...
function installarchlinux(){
	pacstrap /mnt $linuxpkg $microcodepkg $develpkg $baseutilspkg $filesystempkg $networkpkg $audiopkg $bluetoothpkg $xorgpkg $graphicspkg $iopkg $deskenvpkg $guiutilspkg $appspkg 
...
function archenabledisplaymanager(){
	runinchroot "systemctl enable gdm"
}
